...
public class readThread implements Runnable
{
    ...
    public void run() {
        ...
        Server.sendTo("Game Started",readFrom);
        Server.sendTo("Welcome to Tic Tac Toe!",readFrom);
        Server.sendTo("Enter the number of where you want to go",readFrom);
        Server.sendTo("Good Luck against the AI!",readFrom);
        boolean went=false;
        boolean over=false;
        int y;
        while(!over) {
            try{
                went=false;
                while(!went) {
                    Server.sendTo(Server.Display(c),readFrom);
                    try{
                        Thread.sleep(100);
                    }catch(Exception e) {
                    ...
                    in2=in.readUTF();
                    if(c[Integer.parseInt(in2)-1]!="X" && c[Integer.parseInt(in2)-1]!="O") {
                        c[Integer.parseInt(in2)-1]="X";
                        went=true;
                    }
                    else {
                        Server.sendTo("Invalid Move",readFrom);
                    }
                }
                over=winCon(c);
                if(over==true) {
                    Server.sendTo(Server.Display(c),readFrom);
                    Server.sendTo("Game over",readFrom);
                    break;
                }
                y=AiO(c);
                c[y]="O";
    ...
    public static int AiO(String c[]) {
        int y=0;
    ...
    public static int AiX(String c[]) {
        int y=0;
        Random rand=new Random();
        boolean went=false;
        boolean went2=false;
        ...
        if(checkUsedbyO(0,c)==true && checkUsedbyO(8,c)==true && checkUsed(1,c)==false && went==false) {
            y=1;
            went=true;
        }

        if(checkUsedbyO(2,c)==true && checkUsedbyO(6,c)==true && checkUsed(5,c)==false && went==false) {
            y=5;
            went=true;
        }

        if(checkUsedbyO(0,c)==true && checkUsed(8,c)==false && went==false) {
            y=8;
            went=true;
        }
        if(checkUsedbyO(2,c)==true && checkUsed(6,c)==false && went==false) {
            y=6;
            went=true;
        }
        if(checkUsedbyO(8,c)==true && checkUsed(0,c)==false && went==false) {
            y=0;
            went=true;
        }
        if(checkUsedbyO(6,c)==true && checkUsed(2,c)==false && went==false) {
            y=2;
            went=true;
        }

        if(checkUsedbyO(0,c)==true && checkUsed(6,c)==false && went==false) {
            y=6;
            went=true;
        }
        if(checkUsedbyO(2,c)==true && checkUsed(0,c)==false && went==false) {
            y=0;
            went=true;
        }
        if(checkUsedbyO(8,c)==true && checkUsed(2,c)==false && went==false) {
            y=2;
            went=true;
        }
        if(checkUsedbyO(6,c)==true && checkUsed(8,c)==false && went==false) {
            y=8;
            went=true;
        }

        if(checkUsedbyO(4,c)==true && checkUsed(0,c)==false && went==false) {
            y=0;
            went=true;
        }
        if(checkUsedbyO(4,c)==true && checkUsed(2,c)==false && went==false) {
            y=2;
            went=true;
        }
        if(checkUsedbyO(4,c)==true && checkUsed(8,c)==false && went==false) {
            y=8;
            went=true;
        }
        if(checkUsedbyO(4,c)==true && checkUsed(6,c)==false && went==false) {
            y=6;
            went=true;
        }
